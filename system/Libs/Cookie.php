...
class Cookie {
    ...
    /**
     * Ellenőrzi, hogy az ingatlan id szerepel-e a $cookie_name nevű cookie json stringjében
     *
     * @param   string $cookie_name - a cookie neve
     * @param   string $id - az ingatlan id-je
     * @return  bool - true, ha szerepel az id a cookie-ban, false, ha nem
     */
    public static function is_id_in_cookie($cookie_name, $cookie_value)
    {
        $kedvencek_array = json_decode(self::get($cookie_name));

        if (self::exists($cookie_name) && is_array($kedvencek_array)) {
            if (in_array($cookie_value, $kedvencek_array)) {
                return true;
            } else {
                return false;
            }
