...
console.log(global_agents_list);
    ...
    // feldolgozó url-ek
        urlInsert: "admin/datatables/city_insert_update",
        urlUpdate: "admin/datatables/city_insert_update",
        urlDelete: "admin/datatables/city_delete"
    };
    ...
    return {
        ...
        init: function () {
            ...
            vframework.printTable({
                print_button_id: "print_cities", // elem id-je, amivel elindítjuk a nyomtatást 
                table_id: "cities",
                title: "Városok"
            });
        },
...
var DatatableEditable = function () {
    ...
    // ------ ALAPFÜGGVÉNYEK ------------
            ...
            // VÁROS neve oszlop
                jqTds[setup.modCols.city_name].innerHTML = '<input type="text" class="form-control input-small" value="' + aData[setup.modCols.city_name] + '">';
                ...
                */ 
                ...
                // Megye select lista oszlop
                    // megye adatokat tartalmazó tömb
                    var counties = global_county_list;
                    // a megye select lista td eleme
                    var county_select_list_td = jqTds[setup.modCols.county_name];
                    // a select lista td elem data-category-id attribútuma (vagyis a kiválasztott elem)
                    var selectedCountyID = $(county_select_list_td).attr('data-county-id');

                    // régiók option lista
                    var html = '';
                    html += '<select class="form-control select_county"> \r\n';
                    html += '<option value="">-- Válasszon --</option> \r\n';

                    var selected = '';
                    $.each(counties, function(index, value){
                        selected = (selectedCountyID == value["county_id"]) ? 'selected' : '';
                        html += '<option value="' + value["county_id"] + '" ' + selected + '>' + value["county_name"] + '</option> \r\n';
                        selected = '';
                    });

                    html += '</select> \r\n';

                    // Megye neve a td elembe
                    jqTds[setup.modCols.county_name].innerHTML = html;
            ...
            // Város név oszlop update-elése
                oTable.fnUpdate(jqInputs[setup.modCols.city_name].value, nRow, setup.modCols.city_name, false);


            // county elem frissítése
                // a td elem amiben a megye select lista van (ennek az elemnek a data-county-id attribútumát kell majd módosítani)
                var county_select_list_td = $(nRow).find('td')[setup.modCols.county_name];
                var countyName = $('select.select_county :selected').text();
                var countyID = $('select.select_county').val(); // a kiválasztott megye id meghatározása
                // A td elem data-county-id attribútumának módosítása
                $(county_select_list_td).attr('data-county-id', countyID);
                // sor adatok frissítése
                oTable.fnUpdate(countyName, nRow, setup.modCols.county_name, false);
...
//console.log(reference.parents('tr')[0]);          
            ...
            if (nEditing !== null && nEditing != nRow) {
                ...
                bootbox.confirm("Biztosan menteni akarja a módosítást?", function (result) {
                    if (result) {
                        ...
                        // a kiválasztott megye id meghatározása
                        var selected_county_id = $('select.select_county').val();
                        
                        // a kiválasztott referens id meghatározása
                        var selected_agent_id = $('select.select_agent').val();
                        
                        // ha megye option listában nincs kiválasztva semmi
                        if (selected_county_id === '') {
                            App.alert({
                                container: ajax_message, // $('#elem'); - alerts parent container(by default placed after the page breadcrumbs)
                                place: "append", // "append" or "prepend" in container 
                                type: 'danger', // alert's type (success, danger, warning, info)
                                message: 'Nincs megye kiválasztva!', // alert's message
                                close: true, // make alert closable
                                // reset: true, // close all previouse alerts first
                                // focus: true, // auto scroll to the alert after shown
                        ...
                        // Város neve a módósítás után
                        var city_name = reference.closest('tr').find('input').val();
    ...
    return {
        ...
        loadNewCityItems: function(data, langs) {
            _loadNewCityItems(data, langs);
        },

        setObjectData: function(name, data) {
            dataObject.setItem(name, data);
        },

        getObjectData: function(name) {
            dataObject.getItem(name);
        }
